function impos = pascal_data_diff(cls, year)
...
save_file = [cachedir cls '_' dataset_fg '_' year '_diff'];
try
  load(save_file);
catch
  ...
  dataid   = 0;
  for i = 1:length(ids);
    tic_toc_print('%s: parsing positives with difficults (%s %s): %d/%d\n', ...
